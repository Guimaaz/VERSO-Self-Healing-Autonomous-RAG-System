...
class VersoEngine:
    def __init__(self):
        ...
        print(" Verso Engine: Connected to database")
    ...
    def commit_memory(self, fact, id_conversation="session"):
        ...
        print(f"Memory updated: {fact[:50]}...")

    def summarize_and_save(self, history):
        if not history:
            return

        print("Verso: Summarizing the conversation for long-term memory...")
        ...
        self.commit_memory(summary, id_conversation="summary_session_" + os.urandom(2).hex())
    ...
    def answering(self, question, session_history):
        ...
        elif "USE_RAG" in decision or "GENERAL" in decision:
